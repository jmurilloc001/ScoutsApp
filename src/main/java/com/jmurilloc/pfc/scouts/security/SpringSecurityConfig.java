...
public class SpringSecurityConfig
{
    ...
    public SecurityFilterChain filterChain( HttpSecurity http ) throws Exception
    {
        return http.authorizeHttpRequests(
                        authz -> authz.requestMatchers( HttpMethod.GET, "/users", "/meetings", "/affiliates", "/users/{username}/roles", "/news", "/news/{id}", "/news/paginated", "/send-email",
                                "/send-email-backup" ).permitAll().requestMatchers( HttpMethod.POST, "/users/register" ).permitAll().anyRequest().authenticated() )
    ...
    public CorsConfigurationSource corsConfigurationSource()
    {
        ...
        config.setAllowedOriginPatterns( List.of( "http://localhost:4173" ) );
