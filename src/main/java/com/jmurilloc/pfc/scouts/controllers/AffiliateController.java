...
public class AffiliateController
{
    ...
    public List<AffiliateDto> index()
    {
        ...
        affiliates.forEach( affiliate -> affiliateDtos.add( BuildDto.buildDto( affiliate ) ) );
    ...
    public AffiliateDto findById( @PathVariable Long id )
    {
        ...
        if( optionalAffiliate.isPresent() )
        {
            ...
            return BuildDto.buildDto( affiliate );
        }
        throw new AffiliateNotFoundException( MessageError.AFFILIATE_NOT_FOUND.getValue() );
    }
    ...
    public ResponseEntity<String> delete( @PathVariable Long id )
    {
        ...
        if( optionalAffiliate.isPresent() )
        {
            ...
            return ResponseEntity.status( HttpStatus.NOT_FOUND.value() ).body( MessageError.AFFILIATE_NOT_FOUND.getValue() );
    ...
    public ResponseEntity<Object> update( @Valid @RequestBody Affiliate affiliate, BindingResult result, @PathVariable Long id )
    {
        ...
        if( optionalAffiliate.isPresent() )
        {
            ...
            {
                a.setUser( affiliate.getUser() );
            }
            if( affiliate.getBirthday() != null )
            {
                a.setBirthday( affiliate.getBirthday() );
            }
            
            service.save( a );
            
            return ResponseEntity.status( HttpStatus.CREATED ).body( BuildDto.buildDto( a ) );
        }
    ...
    public ResponseEntity<Object> updateImportantData( @Valid @RequestBody Affiliate affiliate, BindingResult result, @PathVariable Long id )
    {
        ...
        if( optionalAffiliate.isPresent() )
        {
            ...
            if( affiliate.getInscripcionDate() != null )
            {
                a.setInscripcionDate( affiliate.getInscripcionDate() );
            }
            if( affiliate.getReuniones() != null )
            {
                a.setReuniones( affiliate.getReuniones() );
            }
            a.setSeccion( affiliate.getSeccion() );
            
            service.save( a );
            
            return ResponseEntity.ok( BuildDto.buildDto( a ) );
        }
        throw new UserNotFoundException( MessageError.AFFILIATE_NOT_FOUND.getValue() );
    }
